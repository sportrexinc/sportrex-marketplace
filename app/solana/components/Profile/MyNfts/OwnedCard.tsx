...
const OwnedCard = ({
  ...
  const handleNavigate = () => {
    router.push(`/solana/nft/${item.token_address}/${item.token_id}`);
  };
    ...
    <div>
      <>
        <div className="w-full overflow-hidden box-border md:h-[350px] h-full bg-blue-header rounded-[10px] p-2 lg:p-4 md:rounded-[20px] flex flex-col  items-start space-y-[12px] justify-between relative">
          ...
          <Link
            href={`/solana/nft/${item.token_address}/${item.token_id}`}
          ...
          <div className="flex flex-1 overflow-hidden justify-between items-center w-full mb-2 px-2">
            <Link href={`/solana/nft/${item.token_address}/${item.token_id}`}>
      ...
      <ShareModal
        ...
        url={`https://sportrex-marketplace-18bv.vercel.app/solana/nft/${item?.token_address}/${item?.token_id}`}
      />
    </div>
  );
};
